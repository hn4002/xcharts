...
export default class XOhlcBar {
    ...
    _draw(data) {
        ...
        if (hlc_only) {
            ...
            this.ctx.moveTo(Math.floor(data.x - halfwidth - line_width_half - 1), Math.floor(data.c))
            this.ctx.lineTo(Math.floor(data.x - line_width_half), Math.floor(data.c))
        } else {
            // Draw open notch with open price
            this.ctx.moveTo(Math.floor(data.x - halfwidth - line_width_half - 1), Math.floor(data.o))
            this.ctx.lineTo(Math.floor(data.x - line_width_half), Math.floor(data.o))
        }

        // Draw close notch
        this.ctx.moveTo(Math.floor(data.x - line_width_half), Math.floor(data.c))
        this.ctx.lineTo(Math.floor(data.x + halfwidth - line_width_half + 1), Math.floor(data.c))
